...
الأفضل إن كل مشاريعك تكون داخل بيئة Linux نفسها لتحصل على أداء أفضل وتجنب مشاكل الصلاحيات.
...
ثم أنشئ مجلد يجمع كل مشاريعك:

```bash
mkdir apps
```

ليصبح المسار كالتالي:

```
/home/mahmoud/apps
```

ومن الآن فصاعدًا، كل مشروع جديد أنشئه داخل هذا المجلد.
...
1. [حمّل Cmder](https://cmder.app/).
2. ادخل إلى **Settings → Startup → Tasks**.
3. أنشئ Task جديد باسم: `Ubuntu`
4. ضع الأمر التالي داخله:

```bash
wsl -d Ubuntu
```

5. من **General → Choose your startup task** اختر التاسك الجديد.
...
يمكننا تعديله مرة واحدة داخل ملف الإعدادات.
...
الآن عند إعادة فتح Cmder ستجد نفسك تلقائيًا داخل مجلد المشاريع بهذا الشكل:

```bash
mahmoud@SherifAdel [apps] ➜
```

وهكذا أصبحت بيئة العمل جاهزة وسريعة بدون خطوات متكررة.
